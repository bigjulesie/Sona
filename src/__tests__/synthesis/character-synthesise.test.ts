...
describe('computeWeightedConfidence', () => {
  it('weights direct_quote higher than inferred', () => {
    const quoteWeight = computeWeightedConfidence({
      ...
      source_perspective: 'first_person',
    })
  ...
  it('weights first_person higher than third_party', () => {
    const firstWeight = computeWeightedConfidence({
      raw_confidence: 0.8,
    ...
    const thirdWeight = computeWeightedConfidence({
      raw_confidence: 0.8,
      evidence_type: 'behavioural_pattern',
      source_type: 'interview',
      source_date: new Date(),
      source_perspective: 'third_party',
    })
    expect(firstWeight).toBeGreaterThan(thirdWeight)
  })
})
...
describe('groupEvidenceByDimension', () => {
  it('groups evidence rows by dimension_key', () => {
    const rows = [
      { dimension_key: 'openness', dimension_category: 'personality', evidence_text: 'A', evidence_type: 'direct_quote', confidence: 0.9, source_type: 'article', source_date: null, source_perspective: 'first_person' },
      { dimension_key: 'openness', dimension_category: 'personality', evidence_text: 'B', evidence_type: 'inferred', confidence: 0.5, source_type: 'article', source_date: null, source_perspective: 'first_person' },
      { dimension_key: 'benevolence', dimension_category: 'values', evidence_text: 'C', evidence_type: 'stated_belief', confidence: 0.8, source_type: 'interview', source_date: null, source_perspective: 'third_party' },
    ]
